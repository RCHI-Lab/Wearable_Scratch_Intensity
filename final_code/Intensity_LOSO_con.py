...
def write(output):
    file_object = open(fh_output + 'intensity_con.txt', 'a')
...
for params in combs:
    ...
    filename = fh_output +"intensity_con_labelpreds.pickle"
    ...
    plt.ylabel('MAE (mW)')
    #plt.show()
    plot_title = "Intensity Regression Con. Mic. Only" 
    plt.title(plot_title)

    plt.savefig(fh_output + "intensity_con" + ".png")
    plt.clf()
    write(output)
    n+=1
