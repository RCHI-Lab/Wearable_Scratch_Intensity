...
def write(output):
    file_object = open(fh_output + 'intensity.txt', 'a')
...
for params in combs:
    ...
    filename = fh_output +"intensity_both_labelpreds.pickle"
    ...
    plt.ylabel('MAE (mW)')
    #plt.show()
    plot_title = "Intensity Regression Both Sensors" 
    plt.title(plot_title)

    plt.savefig(fh_output + "intensity_both" + ".png")
    plt.clf()
    write(output)
    n+=1
